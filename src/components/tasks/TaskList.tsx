...
export const TaskList = ({ tasks, canEdit }: TaskListProps) => {
  ...
  return (
    <>
      <h2 className="text-5xl text-gray-800 font-black my-10">Tareas</h2>
      ...
      <DndContext onDragEnd={handleDragEnd}>
        ...
        {Object.entries(groupedTasks).map(([status, tasks]) => (
          <div key={status} className="min-w-[300px] 2xl:min-w-0 2xl:w-1/5">
						<h3 className={`capitalize text-xl font-light border border-t-8 border-slate-300 bg-white p-3 ${statusStyles[status]}`}>{statusTranslation[status]}</h3>
